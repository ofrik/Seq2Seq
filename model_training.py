...
max_encoder_seq_length = max([len(txt) for txt in np.append(np.append(train_vect_eng_sentences, val_vect_eng_sentences),
                                                            test_vect_eng_sentences)])
max_decoder_seq_length = max([len(txt) for txt in np.append(np.append(train_decoder_input_data, val_decoder_input_data),
...
train_vect_eng_sentences = np.array(
    [pad_sequences(sentence, eng_vocab[PAD_TOKEN], max_encoder_seq_length) for sentence in train_vect_eng_sentences])
train_decoder_input_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in train_decoder_input_data])
train_decoder_target_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in train_decoder_target_data])
val_vect_eng_sentences = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_encoder_seq_length) for sentence in val_vect_eng_sentences])
val_decoder_input_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in val_decoder_input_data])
val_decoder_target_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in val_decoder_target_data])
test_vect_eng_sentences = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_encoder_seq_length) for sentence in test_vect_eng_sentences])
test_decoder_target_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in test_decoder_target_data])
test_decoder_input_data = np.array(
    [pad_sequences(sentence, heb_vocab[PAD_TOKEN], max_decoder_seq_length) for sentence in test_decoder_input_data])
...
encoder_cell = tf.contrib.rnn.MultiRNNCell(
    [tf.nn.rnn_cell.LSTMCell(lstm_size, initializer=lstm_initializer) for _ in range(2)])
...
stepwise_cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
    labels=tf.one_hot(decoder_targets, depth=vocab_size_hebrew, dtype=tf.float32),
...
def get_batch(enc_input, dec_input, dec_target, batch_size):
    ...
    while True:
        ...
        if enc_input is not None:
            ...
            feed[encoder_inputs_length] = np.array(
                [len([word for word in sequence if word != 0]) for sequence in enc_input[from_index:to_index]],
                dtype=np.int32)
        if dec_input is not None:
            feed[decoder_inputs] = dec_input[from_index:to_index].T
            feed[decoder_inputs_length] = np.array(
                [len([word for word in sequence if word != 0]) for sequence in dec_input[from_index:to_index]],
                dtype=np.int32)
...
def decode_sequence(sequence, rev_vocab, to_join=True):
    ...
    """
    arr = [rev_vocab[int(index)] for index in sequence if
           rev_vocab[int(index)] != "<PAD>" and rev_vocab[int(index)] != "<EOS>"]
    if to_join:
        return " ".join(arr)
    else:
        return arr
...
log_every_iterations = 250
print_samples_every_iterations = 1000
train_feed_generator = get_batch(train_vect_eng_sentences, train_decoder_input_data, train_decoder_target_data,
                                 batch_size)
validation_feed_generator = get_batch(val_vect_eng_sentences, val_decoder_input_data, val_decoder_target_data,
                                      batch_size)
try:
    for batch in tqdm(range(max_batches), desc="training"):
        if batch % batches_per_epoch == 0:
            print("Epoch %s" % (int(batch / batches_per_epoch)))
            calc_test_bleu_and_loss(sess, int(batch / batches_per_epoch))
        ...
        if batch == 0 or batch % log_every_iterations == 0:
            ...
            mean_validation_bleu, mean_validation_loss = get_validation_bleu_scores(sess, validation_feed_generator,
                                                                                    batches_per_epoch_validation)
            train_writer.add_summary(get_bleu_summary(mean_validation_bleu, batch), batch)
            train_writer.add_summary(
                tf.Summary(value=[tf.Summary.Value(tag="validation_loss", simple_value=mean_validation_loss), ]), batch)
            if batch % print_samples_every_iterations == 0:
                saver.save(sess, "checkpoints/model.ckpt", global_step=batch)
...
# Calcualte the bleu of the translations of the test data
test_sentences = []
for i in tqdm(range(number_of_batches_in_test)):
    fd = next(test_feed_generator)
    predict_ = sess.run(decoder_prediction, fd)
    for i, (inp, pred, exp) in enumerate(zip(fd[encoder_inputs].T, predict_.T, fd[decoder_targets].T)):
        input_sentence = decode_sequence(inp[::-1], rev_eng_vocab)
        output_sentence = decode_sequence(pred, rev_heb_vocab)
        expected_sentence = decode_sequence(exp, rev_heb_vocab)
        score = sentence_bleu([decode_sequence(pred, rev_heb_vocab, False)], decode_sequence(exp, rev_heb_vocab, False),
                              smoothing_function=chencherry.method1)
...
test_df = pd.DataFrame(test_sentences,
                       columns=["english_sentence", "predicted_translation", "expected_translation", "bleu_score"])

test_df.to_csv("test_results.csv", encoding="utf-8", index=False)

test_feed_generator = get_batch(test_vect_eng_sentences, test_decoder_input_data, test_decoder_target_data, batch_size)
test_sentences_encoding = []
fd = next(test_feed_generator)
predict_ = sess.run(encoder_state, fd)
for i in tqdm(range(number_of_batches_in_test)):
    fd = next(test_feed_generator)
    states_ = sess.run(encoder_state, fd)
    for i, (inp, state) in enumerate(zip(fd[encoder_inputs].T, states_[1].c)):
        input_sentence = decode_sequence(inp[::-1], rev_eng_vocab)
        encoded_sentence = state
        test_sentences_encoding.append([input_sentence, encoded_sentence])

test_encoding_df = pd.DataFrame(test_sentences_encoding, columns=["english_sentence", "encoding"])
test_encoding_df.to_csv("test_encoding.csv", encoding="utf8", index=False)
